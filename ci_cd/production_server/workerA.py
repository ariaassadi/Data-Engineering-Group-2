...
def load_NN_Model():
    ...
    #     build_fn=DenseModel, nb_epoch=200, epochs=10, batch_size=25, verbose=True)
    # estimator.model = load_model("NN_model.h5")
    json_file = open('model.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    estimator = model_from_json(loaded_model_json)
...
def get_predictions():
    ...
    #     print('%s => %f (expected %f)' %
    #           (X[i:i+1].values.tolist(), predictions[i], y[i:i+1]))
